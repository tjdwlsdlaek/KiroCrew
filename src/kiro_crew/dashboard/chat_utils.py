...
def slack_mirror_is_paused(state: Any, session_key: str) -> bool:
    ...
    The channel compaction notice is also not gated, because it cannot reach a
    paused link: it fires only for a channel-ORIGIN session delivering into its
    own home conversation, and an origin carries no disconnect control.
...
def mirror_is_paused(state: Any, session_key: str) -> bool:
    """True when a turn must NOT be mirrored to the session's non-Slack channel.
