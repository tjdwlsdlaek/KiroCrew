...
class CrewOrchestrator:
    ...
    async def purge_slot(self, slot_key: str) -> None:
        ...
        if st is not None:
            for e in st.queue:
                did = str(e.get("dispatch_id") or "")
                if did and e.get("state") not in _TERMINAL_STATES and did not in owned:
                    owned.append(did)
        ...
        target = st.dir if st is not None else data_home() / "crew" / _store_name(slot_key)
        self._stores.pop(slot_key, None)
        for book in (self._locks, self._drain_locks, self._rerun, self._decide_attempts):
