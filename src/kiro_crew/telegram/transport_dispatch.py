...
class TelegramDispatcher:
    ...
    def _bind_origin_mirror(
        ...
        last exactly until the user's next message. Opting out is treated as the
        INVARIANT "THIS chat is not mirrored into", not merely as "do not write":
        ``/unlink`` needs two session-map saves (the flag, then the release), and
        an interruption between them leaves the flag set with the binding still
        live. Only declining to write would make that state permanent — the flag
        suppresses the very code that would otherwise correct the binding — so
        replies would keep arriving in a chat the user switched off. Clearing
        here is what repairs it, on the next turn, whatever the interruption.

        Scoped to the binding this method would itself have written. A mirror
        pointing somewhere ELSE is a target the user chose explicitly (the
        dashboard can bind a surfaced session to any conversation), and an
        opt-out for this chat says nothing about it — deleting it would silently
        cut off a link the user had just made.
        """
        ...
        if self.sessions.mirror_opt_out(session_key):
            if self.sessions.get_mirror_link(session_key) == desired:
                self.sessions.clear_mirror_link(session_key)
            return
        if self.sessions.get_mirror_link(session_key) == desired:
            return
        try:
