...
case "$url" in
  ...
  *"/commits/"*"/status"*)
    ...
    # final value). No fixture = no status exists.
    if [ -f "$FIXTURES/existing_status_state.txt" ]; then
      cat "$FIXTURES/existing_status_state.txt"
    fi
    exit 0 ;;
...
class TestPersistentTransportFailureIsNonTerminal:
    ...
    def test_a_truncated_run_defers_to_an_existing_terminal_verdict(
        self, runner: Runner
    ):
        # Two runs can evaluate the same revision. A delayed run whose reads
        # died must not publish its pending fallback OVER the terminal
        # verdict a concurrent complete run already posted -- that would
        # re-block a green PR (or hide a red behind "checking"). It defers:
        # exit 0, no publishable verdict.
        proc, outputs = runner.evaluate(
