...
out of a file. `apps/issue-radar/data/config.json` is therefore on
`security._WRITE_PROTECTED_HOME_PATHS` — agent file and shell tools cannot modify
it, while reads stay allowed (it holds no secret and the app reads it on every
request). Two authorization inputs live in that file: `repos[]` is the
...
vouching for the agent's own choice. The store opens the path directly and does
not route through the gate, so the dashboard's own writes still work.
